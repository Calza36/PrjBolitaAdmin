...
class PlayerManageWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    printS("defaultTabIndex-> $defaultTabIndex");
     printS("GetItMock.user.userType!-> ${GetItMock.typeOfUser()}");
    if(GetItMock.typeOfUser() == "GeneralCollector"){
      return   Scaffold(
        appBar: AppBar(title: const Text("Colectores")),body: const PlayersCollectors(),

      );
    }
    if(GetItMock.typeOfUser() == "Collector"){
      return   Scaffold(
        appBar: AppBar(title: const Text("Listeros")),body:
      const PlayersFootCollectors());
    }
